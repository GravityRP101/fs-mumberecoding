...
BadWords = ["nigger", "gringo", "faggot", "retard", "fag ", "nigga"]

# Mumble ip Where It Needs To Connect
host = "127.0.0.1"

#Mumble port Where It Needs To Connect
port = 30120

#This Is Your RCON Password For The FiveM Server. Set to False to disable.
RconPassword = 'False'
...
# Voice Model To Use When Transcribing Speech, Replace To False To Disable Transcriptions.
...
def IsSpeechBad(text):
	...
	for bad in BadWords:
		if bad.lower() in text.lower():
			BadWordssDetected.append(bad)

	if len(BadWordssDetected) > 0:
		return BadWordssDetected
	else:
		return False

def LogToDiscordBad(username, sentence, badSpeech, clipFilename):
	if BadWebhook and BadWebhook != "":
		webhook = DiscordWebhook(url=BadWebhook, rate_limit_retry=True, content=username+": \""+sentence+"\", Detected bad text: "+(', '.join(badSpeech)))
...
def LogToDiscordGood(username, sentence, clipFilename):
	if GoodWebhook and GoodWebhook != "":
		webhook = DiscordWebhook(url=GoodWebhook, rate_limit_retry=True, content=username+": \""+sentence+"\"")
		with open(clipFilename, "rb") as f:
			webhook.add_file(file=f.read(), filename=username+'.mp3')
		response = webhook.execute()
...
def ProcessAudio(userName, userID, filename):
	...
	if re.search("[a-zA-Z]", whatWasSaid):
		BadWords = IsSpeechBad(whatWasSaid)
		SendRcon({"Username": userName, "Message": whatWasSaid})
		if BadWords:
			LogToDiscordBad(userName, whatWasSaid, BadWords, filename)
		else:
